...
void NazaCanDecoderLib::begin()
{
  ...
  for(int i = 0; i < 3; i++) CAN.setFilter(FILTER_090, i);
  for(int i = 3; i < 6; i++) CAN.setFilter(FILTER_108, i);
  for(int i = 6; i < 8; i++) CAN.setFilter(FILTER_7F8, i);
}

double NazaCanDecoderLib::getLat() { return lat; }
double NazaCanDecoderLib::getLon() { return lon; }
double NazaCanDecoderLib::getAlt() { return alt; }
double NazaCanDecoderLib::getGpsAlt() { return gpsAlt; }
double NazaCanDecoderLib::getSpeed() { return spd; }
NazaCanDecoderLib::fixType_t NazaCanDecoderLib::getFixType() { return fix; }
uint8_t NazaCanDecoderLib::getNumSat() { return sat; }
double NazaCanDecoderLib::getHeading() { return heading; }
double NazaCanDecoderLib::getHeadingNc() { return headingNc; }
double NazaCanDecoderLib::getCog() { return cog; }
double NazaCanDecoderLib::getVsi() { return vsi; }
...
uint8_t NazaCanDecoderLib::getDay() { return day; }
uint8_t NazaCanDecoderLib::getHour() { return hour; }
uint8_t NazaCanDecoderLib::getMinute() { return minute; }
uint8_t NazaCanDecoderLib::getSecond() { return second; }
uint16_t NazaCanDecoderLib::getBattery() { return battery; }
int16_t NazaCanDecoderLib::getRcIn(NazaCanDecoderLib::rcInChan_t chan) { return rcIn[chan]; }
...
uint16_t NazaCanDecoderLib::decode()
{
  ...
  {
    CAN.read(canMsg);
    if(canMsg.id == 0x090) canMsgIdIdx = 0;
    else if(canMsg.id == 0x108) canMsgIdIdx = 1;
    else if(canMsg.id == 0x7F8) canMsgIdIdx = 2;
